...
export function ArtistCharts({ snapshots }: { snapshots: Snapshot[] }) {
  ...
  return (
    <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
      <div>
        <h3 className="text-sm text-white/35 mb-3">Followers</h3>
      ...
      <div>
        <h3 className="text-sm text-white/35 mb-3">Views</h3>
        <ResponsiveContainer width="100%" height={200}>
          <LineChart data={data}>
            <CartesianGrid strokeDasharray="3 3" stroke="rgba(255,255,255,0.04)" />
            <XAxis dataKey="date" tick={{ fill: "rgba(255,255,255,0.35)", fontSize: 10 }} />
      ...
      <div className="md:col-span-2">
        <h3 className="text-sm text-white/35 mb-3">Engagement Rate (%)</h3>
        {hasEngagement ? (
          <ResponsiveContainer width="100%" height={200}>
            <LineChart data={data}>
              <CartesianGrid strokeDasharray="3 3" stroke="rgba(255,255,255,0.04)" />
          ...
          <div className="text-xs text-white/35">No engagement data available yet.</div>
        )}
      </div>
    </div>
  );
